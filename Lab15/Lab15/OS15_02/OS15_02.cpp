...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		while (WaitForSingleObject(hStopEvent, 0) == WAIT_TIMEOUT)
		{
			...
			Element* el;
			Element* el1 = OS15::HTElement::CreateElementInsert(h1, key.c_str(), key.length() + 1, "0", 2);

			if (!OS15::DataManipulator::Insert(h1, HT, el1))
			{
				std::cout << OS15::Utils::getLastError(h1, HT) << std::endl;
			}
			else if ((el = OS15::DataManipulator::Get(h1, HT, el1)) == NULL)
			{
				std::cout << OS15::Utils::getLastError(h1, HT) << std::endl;
			}
			else if (el)
